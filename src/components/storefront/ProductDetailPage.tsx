...
export const ProductDetailPage: React.FC<{
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      {/* Product Viewport */}
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 bg-slate-900/90 border border-slate-800 rounded-3xl p-6">
        ...
        <div className="md:col-span-6 space-y-3">
          <div className="relative rounded-2xl overflow-hidden bg-slate-950 border border-slate-800 h-80 sm:h-96">
            ...
            />
            <span className="absolute top-4 left-4 text-xs uppercase font-bold tracking-wider px-3 py-1 rounded-full bg-slate-950/80 backdrop-blur-md text-emerald-400 border border-emerald-500/30">
              {product.category}
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[product.image, product.image, product.image].map((img, i) => (
              <div
                key={i}
                className="h-20 rounded-xl overflow-hidden bg-slate-950 border border-slate-800 cursor-pointer opacity-70 hover:opacity-100 transition-opacity"
              >
        ...
        <div className="md:col-span-6 space-y-4 flex flex-col justify-between">
          <div className="space-y-3">
            <div>
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex items-center space-x-1 text-amber-400 text-xs font-bold font-mono hover:underline"
                >
            ...
            <div className="space-y-1.5 pt-2">
              <label className="text-xs font-semibold text-slate-300">
                Colorway: <span className="text-emerald-400 font-normal">{selectedColor}</span>
              </label>
            ...
            <div className="space-y-1.5 pt-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-xs text-emerald-400 hover:text-emerald-300 font-medium flex items-center gap-1 underline"
                >
          ...
          <div className="space-y-2.5 pt-4 border-t border-slate-800">
            <button
              ...
              className={`w-full py-3.5 rounded-2xl text-xs font-bold uppercase tracking-wider flex items-center justify-center space-x-2 transition-all shadow-lg ${
                addedAnimation
                  ...
                  : 'bg-emerald-500 hover:bg-emerald-400 text-slate-950 shadow-emerald-500/30 hover:scale-[1.01]'
              }`}
            >
              {addedAnimation ? (
                <>
            ...
            <button
              onClick={handleAskAI}
              className="w-full py-2.5 rounded-2xl bg-slate-800/80 hover:bg-slate-800 text-slate-200 hover:text-white text-xs font-semibold flex items-center justify-center space-x-2 border border-slate-700 transition-all"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 flex items-center space-x-3">
          ...
          <div className="text-xs">
            ...
            <div className="text-slate-400">Dispatched in 24h via Delhivery Air</div>
          </div>
        </div>
        <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 flex items-center space-x-3">
          <RotateCcw className="h-5 w-5 text-cyan-400 shrink-0" />
          <div className="text-xs">
            <div className="font-bold text-white">7-Day Zero-Risk Return</div>
            <div className="text-slate-400">Doorstep pickup & instant size swap</div>
          </div>
        </div>
        <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 flex items-center space-x-3">
          ...
          <div className="text-xs">
            ...
            <div className="text-slate-400">Guaranteed anti-separation durability</div>
          </div>
        </div>
      </div>
