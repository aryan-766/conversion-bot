...
export const CartDrawer: React.FC<{
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex justify-end animate-fade-in">
      <div className="bg-slate-900 border-l border-slate-800 w-full max-w-md h-full flex flex-col justify-between p-6 shadow-2xl animate-slide-up">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-3 pr-1">
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item, idx) => (
              <div
                key={idx}
                className="p-3 rounded-xl bg-slate-950/70 border border-slate-800 flex items-center justify-between space-x-3 text-xs"
              >
                <img
                  src={item.product.image}
                  alt={item.product.name}
                  className="h-16 w-16 rounded-lg object-cover bg-slate-900 shrink-0"
                />
        ...
        <div className="pt-4 border-t border-slate-800 space-y-3">
          ...
          <form onSubmit={handleApplyCoupon} className="flex items-center space-x-2">
            <div className="relative flex-1">
              <Tag className="h-3.5 w-3.5 text-slate-500 absolute left-3 top-3" />
              <input
                type="text"
                placeholder="Discount code (e.g. SAVE10)"
                value={couponInput}
                onChange={e => setCouponInput(e.target.value)}
                className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-8 pr-3 py-2 text-xs text-white uppercase placeholder:normal-case font-mono focus:outline-none focus:border-emerald-500"
              />
            </div>
            <button
              type="submit"
              className="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold shrink-0"
            >
              Apply
            </button>
          </form>
          ...
          {couponStatus && (
            <div
              className={`text-[11px] font-medium flex items-center gap-1 ${
                couponStatus.success ? 'text-emerald-400' : 'text-rose-400'
              }`}
            >
          ...
          <button
            ...
            className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 text-xs font-bold uppercase tracking-wider flex items-center justify-center space-x-2 transition-all shadow-lg shadow-emerald-500/20 disabled:opacity-40"
          >
